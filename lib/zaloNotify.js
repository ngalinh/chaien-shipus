...
async function sendZaloMessage({ phone, name, message, image, renderUrl }) {
  ...
  const effectiveName = (contact && contact.zalo_name) || name;

  if (contact && contact.report_target) {
    return sendOnce(baseUrl, { phone, name: effectiveName, message, images, renderUrl, notifyTarget: contact.report_target });
  }
  const groupResult = await sendOnce(baseUrl, { phone, name: effectiveName, message, images, renderUrl, notifyTarget: 'group' });
  if (groupResult.ok || !/KHONG_THAY_HOI_THOAI/.test(groupResult.error || '')) return groupResult;
  return sendOnce(baseUrl, { phone, name: effectiveName, message, images, renderUrl, notifyTarget: 'personal' });
}
