...
/**
 * Gửi 1 tin nhắn báo ship qua Zalo và chờ kết quả (poll job tới khi xong hoặc timeout).
 * @param {{phone:string, name?:string, message:string}} p
 * @returns {Promise<{ok:boolean, error?:string}>}
 */
async function sendZaloMessage({ phone, name, message }) {
  const baseUrl = runnerBaseUrl();
  const sendRes = await fetch(`${baseUrl}/api/zalo/send`, {
    method: 'POST',
    headers: headers(),
    body: JSON.stringify({ profile: 'default', keyword: phone, name, message, notifyTarget: 'personal' }),
  });
  if (!sendRes.ok) {
