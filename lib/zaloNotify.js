 ...
 *
 * Tìm theo SĐT khách trước. Nếu không ra hội thoại (KHONG_THAY_HOI_THOAI) VÀ Danh bạ Zalo có
 * zalo_name cho SĐT này, thử lại lần 2 gõ ĐÚNG TÊN hội thoại đó thay vì SĐT — dùng cho case
 * nhiều khách cùng báo chung 1 nhóm/tài khoản Zalo (vd của người thân) mà chính SĐT của họ
 * không tra ra hội thoại đó trên Zalo.
...
async function sendZaloMessage({ phone, name, message, image, renderUrl }) {
  ...
  const byPhone = await attemptTargets(
    baseUrl, { phone, name: (contact && contact.zalo_name) || name, message, images, renderUrl }, targets
  );
  if (byPhone.ok || !(contact && contact.zalo_name) || !/KHONG_THAY_HOI_THOAI/.test(byPhone.error || '')) {
    return byPhone;
  }
