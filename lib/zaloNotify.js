 ...
 * hoặc timeout). Ưu tiên tìm hội thoại NHÓM của khách trước; nếu khách không có nhóm (runner
 * báo KHONG_THAY_HOI_THOAI) mới rơi về tìm hội thoại CÁ NHÂN.
...
async function sendZaloMessage({ phone, name, message, image, renderUrl }) {
  ...
  const groupResult = await sendOnce(baseUrl, { phone, name, message, images, renderUrl, notifyTarget: 'group' });
  if (groupResult.ok || !/KHONG_THAY_HOI_THOAI/.test(groupResult.error || '')) return groupResult;
  return sendOnce(baseUrl, { phone, name, message, images, renderUrl, notifyTarget: 'personal' });
}
