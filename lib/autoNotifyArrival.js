  ...
  for (const b of batches) {
    ...
    try {
      ...
      if (!result.ok) {
        ...
        ).run(b.batch_date, b.customer_id, 'arrival', 'zalo', message, 'failed', result.error || null);
      ...
      const markNotified = db.transaction(() => {
        ...
        ).run(b.batch_date, b.customer_id, 'arrival', 'zalo', message, 'success');
      });
      markNotified();
      ...
      try {
        db.prepare(
          'INSERT INTO notification_log (batch_date, customer_id, type, channel, message, status, error) VALUES (?, ?, ?, ?, ?, ?, ?)'
        ).run(b.batch_date, b.customer_id, 'arrival', 'zalo', message, 'failed', e.message);
