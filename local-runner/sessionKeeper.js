 ...
 *
 * Zalo Basso chỉ giữ session ~1 tuần cho mỗi profile. Module này định kỳ mở TỪNG profile
 * CÓ lưu tài khoản/mật khẩu, gọi checkLoggedIn — hàm này tự đăng nhập lại nếu thấy form
 * login — nên session luôn tươi. Chỉ chạy khi CHAIEN_SESSION_KEEPALIVE=true.
 *
...
function keepableAccounts() {
  return accountsStore.list().filter((a) => profileExists(a.key) && loginCredsFor(a.key));
}
...
/** Khởi động vòng lặp giữ ấm (no-op nếu CHAIEN_SESSION_KEEPALIVE tắt). */
function start() {
